...
// Lista de aulas autorizadas. Podés añadir más líneas aquí abajo según crezca
// la institución.
const int MAX_AULAS = 5;
DispositivoAula tablaAulas[MAX_AULAS] = {
    {"AULA_A",
     {0xB0, 0xCB, 0xD8, 0x8E, 0x72, 0x74}}, // MAC de AULA_A para la prueba
    {"AULA_B",
     {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}}, // Siguiente mac para aula B
...
bool enviarHuellaPorEspNow(uint8_t *macDestino, int idSync, int idHuella,
                           String ci, String roomId, String tipo,
                           String huella);
...
void onReceive(const esp_now_recv_info_t *recv_info,
  ...
  if (len == sizeof(PaqueteAsistencia)) {
    memcpy((uint8_t *)&asistenciaRecibida, incomingData,
           sizeof(PaqueteAsistencia));
    hayAsistenciaPendiente =
        true; // Despierta la acción en el loop() principal de forma segura
  }
}
...
void setup() {
  ...
  Serial.print("Canal de Wi-Fi del Router: ");
  ...
  for (int i = 0; i < MAX_AULAS; i++) {
    ...
    if (tablaAulas[i].mac[0] == 0x00 && tablaAulas[i].mac[5] == 0x00)
      continue;
  ...
  server.on("/sync", HTTP_POST, []() {
    ...
    server.send(200, "application/json",
                "{\"status\":\"ok\",\"message\":\"Gateway recibio orden de "
                "sincronizar\"}");
    pedirSyncPendiente();
  });
...
void loop() {
  ...
  // Control Asíncrono de Seguridad: Despacha la asistencia fuera de la
...
void pedirSyncPendiente() {
  ...
  HTTPClient http;
  String url =
      BASE + "src/api/Gateway/obtener_sync_aula.php?room_id=" + ROOM_ID;
  ...
  if (!buscarMacPorAula(roomId, macObjetivo)) {
    Serial.printf("Error Critico: No existe registro de direccion MAC para el "
                  "aula '%s'\n",
                  roomId.c_str());
    confirmarSync(idSync, "ERROR",
                  "El Gateway no tiene mapeada la direccion MAC de esa aula");
    http.end();
    return;
  }

  // Ejecución del disparo pasándole la MAC encontrada en la tabla
  bool enviado = enviarHuellaPorEspNow(macObjetivo, idSync, idHuella, ci,
                                       roomId, tipo, huella);

  if (enviado) {
    confirmarSync(idSync, "CONFIRMADO",
                  "Huella enviada e instalada en el aula de forma integra");
  } else {
    confirmarSync(idSync, "ERROR",
                  "Fallo el enlace de radio de ESP-NOW entre pisos");
  }
...
bool enviarHuellaPorEspNow(uint8_t *macDestino, int idSync, int idHuella,
  ...
  Serial.printf(
      "Disparando ráfaga hacia la MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
      macDestino[0], macDestino[1], macDestino[2], macDestino[3], macDestino[4],
      macDestino[5]);
  ...
  for (int i = 0; i < total; i++) {
    ...
    String parte = huella.substring(
        i * chunkSize, min((i + 1) * chunkSize, (int)huella.length()));
...
void confirmarSync(int idSync, String estado, String mensaje) {
  ...
  String body =
      "id_sync=" + String(idSync) + "&estado=" + estado + "&mensaje=" + mensaje;
  http.POST(body);
  http.end();
}
...
void procesarEnvioAsistenciaBackend(PaqueteAsistencia datos) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println(
        "Imposible procesar asistencia: Conexión Wi-Fi perdida temporalmente.");
    return;
  }
  ...
  // Armamos el cuerpo del POST urlencoded mapeando los datos de la huella
  // ingresada
  String body = "room_id=" + String(datos.room_id) + "&ci=" + String(datos.ci) +
