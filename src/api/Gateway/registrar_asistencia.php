...
require_once __DIR__ . '/../../classes/ClaseDiaria.php';
...
$evento->timestamp_evento=$momento; $evento->origen_node_id='GATEWAY_ESP32'; $evento->sincronizado=1;
try {
    $db->beginTransaction();
    if (!$evento->crear()) throw new RuntimeException('No se pudo registrar el evento.');
    $idClase = null;
    if ($tipo === 'profesor') {
        $idClase = (new ClaseDiaria($db))->procesarMarcaProfesor($userId, (int) $id_aula, $momento);
    }
    $db->commit();
    echo json_encode([
        "status"=>"success",
    ...
    echo json_encode(["status"=>"error", "message"=>"Error al guardar evento"]);
}
