    ...
    public function detect(string $text): array
    {
        ...
        $rules = [
            ...
            'EXCEL_AVANZADO' => [
                ...
                ['Procedimientos',8,'/PROCEDIMIENTOS/'], ['Instrumentos',8,'/INSTRUMENTOS/'],
                ['Códigos de tema',10,'/TEMA\s+\d+\.\d+/'], ['Códigos de indicador',10,'/\b\d+\.\d+\.\d+\b/'],
            ],
            'COMPETENCIA_CONTENIDO' => [
                ...
                ['Fecha',5,'/\bFECHA\b/'], ['Días de clase',5,'/DIAS DE CLASE/'],
            ],
        ];
        $scores = [];
        ...
        $formats = array_keys($scores); $best = $formats[0]; $second = $formats[1];
        $margin = $scores[$best]['score'] - $scores[$second]['score'];
        $recognized = $scores[$best]['score'] >= self::MIN_SCORE && $margin >= self::MIN_MARGIN;
        return [
            'format'=>$recognized ? $best : null,
            'error_type'=>$recognized ? null : 'UNKNOWN_FORMAT',
            'threshold'=>self::MIN_SCORE, 'minimum_margin'=>self::MIN_MARGIN,
            'winning_score'=>$scores[$best]['score'], 'margin'=>$margin, 'scores'=>$scores,
        ];
    }
}
